...
SDK_BIN=${PWD}/appinventor/appengine/build
echo "SDK_BIN is: $SDK_BIN"
WAR_DIR="$SDK_BIN/war"
JAR_FILE="appinventor/lib/appengine/appengine-java-sdk-1.9.17/lib/appengine-tools-api.jar"


WEB_APP="--port=8888 --address=0.0.0.0"
if [ ! -e "$JAR_FILE" ]; then
...
java -ea -cp "$JAR_FILE" \
  ...
  com.google.appengine.tools.development.DevAppServerMain "$@" $WEB_APP $WAR_DIR
